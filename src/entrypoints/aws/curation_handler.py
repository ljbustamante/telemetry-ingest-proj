...
def _run_etl(event: Dict[str, Any] | None = None) -> Dict[str, Any]:
...
def handle(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    if method == "GET" and "curation" in path:
        return _json_response(200, {"ok": True, "service": "curation-etl"}, event)
    ...
    if method == "POST" and "curation" in path and "run" in path:
        ...
        if not secret:
            ...
            return _json_response(503, {"error": "curation_http_not_configured"}, event)
        hdrs = _headers(event)
