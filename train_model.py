...
BASE_DIR = "snapshots"
DEVICE_SUMMARY_CSV = "device_risk_summary_train.csv"
MIN_SNAPSHOTS_PER_DEVICE = 5
RECENT_WINDOW = 50
...
def compute_risk_points(row):
    """
    Asigna puntos de riesgo a UN snapshot, combinando:
    - rareza estadística (Isolation Forest)
    - señales de fallo de hardware (disco, WHEA, BSOD, etc.)
    - estrés térmico (CPU/GPU)
    """
    rp = 0
    score = row.get("anomaly_score", 0)

    # 1) Rareza (Isolation Forest)
    if score < -0.30:
        rp += 4
    elif score < -0.15:
        rp += 2
    ...
    # 4) Disco: salud + SMART + eventos 153/129/55
...
def compute_prob_failure_snapshot(row):
    """
    Probabilidad de falla para UN snapshot (0–1),
    combinando rareza estadística + puntos de riesgo.
    Ajustada para que "Alto" solo aparezca cuando es realmente grave.
    """
    ...
    # Escalamos los puntos de riesgo para que aporten como máximo ~0.5
    rp_scaled = min(rp, 10.0) / 20.0  # 0–0.5
...
def device_risk_level_from_prob(p: float) -> str:
    """
    Define Bajo / Medio / Alto solo a partir de la probabilidad de falla
...
def pct(mask: pd.Series) -> float:
    """Porcentaje (0–100) de True en una serie booleana; 0 si está vacía."""
...
def analyze_device(df_device_in: pd.DataFrame, device_id: str) -> dict:
    """
    Entrena Isolation Forest SOLO con los snapshots de este device,
    calcula anomalías y riesgo, y devuelve una fila de resumen explicativa
    para usar en monitoreo predictivo.
    """
    ...
    if total_snapshots >= MIN_SNAPSHOTS_PER_DEVICE:
    ...
    df["prob_failure_snapshot"] = df.apply(
    ...
    n_recent = min(RECENT_WINDOW, total_snapshots)
    ...
    bugcheck_7d_max_recent = (
        int(bug7d_recent.max()) if len(bug7d_recent) > 0 and bug7d_recent.notna().any() else 0
    )
    ...
    summary_row = {
        ...
        "failure_risk_historical": float(failure_risk_historical) if failure_risk_historical is not None else 0.0,
        ...
        "avg_cpu_temp_max_recent": float(avg_cpu_temp_max_recent) if avg_cpu_temp_max_recent is not None else "",
        "max_cpu_temp_max_recent": float(max_cpu_temp_max_recent) if max_cpu_temp_max_recent is not None else "",
        "pct_high_cpu_temp_recent": float(pct_high_cpu_temp_recent),
        "pct_throttling_recent": float(pct_throttling_recent),
        "min_disk_health_score_recent": float(min_disk_health_recent) if min_disk_health_recent is not None else "",
        "avg_disk_health_score_recent": float(avg_disk_health_recent) if avg_disk_health_recent is not None else "",
...
def main():
    print(f"[INFO] Leyendo JSON desde carpeta base '{BASE_DIR}'...")
    df_all = load_snapshots(BASE_DIR)

    if "device_id" not in df_all.columns:
        raise RuntimeError("No se encontró la columna 'device_id' en los datos.")

    execution_time = datetime.utcnow().isoformat()

    summaries = []
    for device_id, df_device in df_all.groupby("device_id"):
        print(f"[INFO] Analizando device_id={device_id} con {len(df_device)} snapshots...")
        summary = analyze_device(df_device, device_id)
        summary["execution_time"] = execution_time
        summaries.append(summary)

    device_summary_df = pd.DataFrame(summaries)

    device_summary_df = device_summary_df.sort_values(
        "predicted_failure_risk", ascending=False
    )

    device_summary_df.to_csv(DEVICE_SUMMARY_CSV, index=False, encoding="utf-8-sig")
    print(f"[INFO] Resumen predictivo por device_id guardado en '{DEVICE_SUMMARY_CSV}'")

    print("\nTop devices por predicted_failure_risk:")
    print(device_summary_df.head(10).to_string(index=False))
